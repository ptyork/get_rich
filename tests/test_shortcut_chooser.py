...
def test_shortcut_chooser_invalid_styles_and_messages():
    bad_styles = {"not_a_style": 123, "shortcut_prefix_style": 42}
    bad_messages = {"not_a_message": 123, "shortcut_select_key": 42}
    chooser = ShortcutChooser(
        choices=["a"], styles=bad_styles, messages=bad_messages, console=fake_console()
    )
    # Should use the value as-is
    assert chooser.styles.shortcut_prefix_style == 42
    assert chooser.messages.shortcut_select_key == 42


def test_shortcut_chooser_unicode_choices_and_messages():
    custom_messages = {"shortcut_select_key": "Appuyez sur une touche 🚀"}
    chooser = ShortcutChooser(
    ...
    assert "🚀" in chooser.messages.shortcut_select_key
    assert "🍕" in [str(c) for c in chooser.choices]


def test_shortcut_chooser_on_key_and_hooks():
    called = {
        "before": False,
        "after": False,
        "change": False,
