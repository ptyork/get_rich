...
class FileChooser(FilterChooser):
    ...
    def _prepare_choices(self) -> None:
        ...
        options = []
        if self.choose_dirs:
            options.append(Text.from_markup(f"SELECT {path}{sep}"))
        else:
            options.append(
                Text.from_markup(f".{sep}    [{self.styles.full_path_style}]({path})[/]")
            )
        if path.parent and path.parent != path:
            options.append(
                Text.from_markup(f"..{sep}   [{self.styles.full_path_style}]({path.parent})[/]")
            )
        elif IS_WINDOWS and path == Path(path.anchor):
            # On Windows, at root of drive, show other drives
            drives = _get_drives_win32()
            if drives:
                drive_choices = []
                for drive in drives:
                    if Path(drive) != path:
                        drive_choices.append(
                            Text.from_markup(f"{drive}")
                        )
                options = drive_choices + options
        ...
        if not self.choose_dirs:
            ...
            if all:
                selected_index = len(options)  # first non-special entry

        options.extend([Text.from_markup(name) for name in all])
        if not self.choose_dirs and selected_file and (selected_file in files):
            selected_index = options.index(Text.from_markup(selected_file))  # type: ignore
        ...
        if self.auto_filter and len(self.choices) <= self.height:
    ...
    def __init__(
        ...
        *,
        ...
        # Parent already called _prepare_choices() which builds file list
